...
const DashboardHeader: FC = () => {
  ...
  const [audio] = useState(
    () => typeof window !== "undefined" ? new Audio("https://res.cloudinary.com/dkg6jv4l0/video/upload/v1716750964/notification_jvwqd0.mp3") : null
  );

  const playNotificationSound = () => { audio?.play(); };

  useEffect(() => {
    // THE DEFINITIVE FIX: Check for the nested 'notifications' property
  ...
  useEffect(() => {
    ...
    return () => { socketId.off("newNotification"); };
  }, [refetch]);
    ...
    <div className="w-full flex items-center justify-end p-6 fixed top-5 right-0 z-[9999999]">
      ...
      {open && (
        <div className="w-[350px] h-[60vh] overflow-y-scroll py-3 px-2 border border-[#ffffff0c] dark:bg-[#111C43] bg-white shadow-xl absolute top-16 z-[1000000000] rounded">
          ...
          {/* Use the state variable which is guaranteed to be an array */}
